...
// Дополнительные функции к стандартным БСП
// Добавлено SVO 20/09/2023
#Область ДополнительныеФункции

// Удаляет пробелы справа и слева в строке, а также удаляет повторяющиеся пробелы внутри строки; 
//
// Параметры:
//  Строка	        - Строка - исходная строка, из которой необходимо удалить лишние пробелы;
//
Процедура УдалитьЛишниеПробелы(Строка) Экспорт  
...
Процедура УдалитьЛишниеСимволы(Строка) Экспорт
	
	Строка = СтрЗаменить(Строка, Символы.ПС, "");  
	Строка = СтрЗаменить(Строка, Символы.ВК, "");  
	Строка = СтрЗаменить(Строка, Символ(160), " "); 
	СтроковыеФункцииКлиентСервер.УдалитьЛишниеПробелы(Строка);  
	
КонецПроцедуры


//В проверяемой строке есть символы русской или латинской раскладки или цифры
Функция ВСтрокеЕстьБуквыИлиЦифры(Строка) Экспорт 

	Для НомерСимвола = 1 По СтрДлина(Строка) Цикл 
		
		Если (КодСимвола(Строка, НомерСимвола) >= 48 И КодСимвола(Строка, НомерСимвола) <= 57)
			ИЛИ (КодСимвола(Строка, НомерСимвола) >= 65 И КодСимвола(Строка, НомерСимвола) <= 90)
			ИЛИ (КодСимвола(Строка, НомерСимвола) >= 97 И КодСимвола(Строка, НомерСимвола) <= 122)
			ИЛИ (КодСимвола(Строка, НомерСимвола) >= 1040 И КодСимвола(Строка, НомерСимвола) <= 1103) Тогда
